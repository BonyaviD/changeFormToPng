...
.password-wrap{position:relative}.password-wrap input{padding-right:48px}.password-toggle{position:absolute;right:7px;top:50%;transform:translateY(-50%);display:grid;width:34px;height:34px;place-items:center;border:0;border-radius:6px;background:transparent;color:#565d72;padding:0;cursor:pointer}.password-toggle:hover{background:#eef1fb;color:#1f3fc3}.password-toggle:focus-visible{outline:2px solid #1f3fc3;outline-offset:1px}.password-toggle svg{width:18px;height:18px}.password-toggle .eye-off{display:none}.password-toggle[aria-pressed="true"] .eye-open{display:none}.password-toggle[aria-pressed="true"] .eye-off{display:block}
...
<label for="password">رمز عبور</label><div class="password-wrap"><input id="password" name="password" type="password" dir="ltr" autocomplete="current-password" maxlength="256" required><button class="password-toggle" id="toggle-password" type="button" aria-label="نمایش رمز عبور" aria-controls="password" aria-pressed="false" title="نمایش رمز عبور"><svg class="eye-open" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg><svg class="eye-off" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="m3 3 18 18M10.6 10.7a2 2 0 0 0 2.7 2.7M9.9 4.2A10.8 10.8 0 0 1 12 4c6.5 0 10 8 10 8a16.7 16.7 0 0 1-2.1 3.2M6.6 6.6C3.5 8.5 2 12 2 12s3.5 8 10 8a9.8 9.8 0 0 0 4.3-1"/></svg></button></div>
...
<script nonce="${escapeHtml(nonce)}">(()=>{const i=document.getElementById("password"),b=document.getElementById("toggle-password");if(!(i instanceof HTMLInputElement)||!(b instanceof HTMLButtonElement))return;b.addEventListener("click",()=>{const show=i.type==="password";i.type=show?"text":"password";const label=show?"مخفی کردن رمز عبور":"نمایش رمز عبور";b.setAttribute("aria-label",label);b.setAttribute("title",label);b.setAttribute("aria-pressed",String(show));i.focus({preventScroll:true})})})();</script></body></html>`;
}
